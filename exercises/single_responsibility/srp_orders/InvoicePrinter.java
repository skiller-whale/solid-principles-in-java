...
public class InvoicePrinter {
    ...
    public InvoicePrinter(CustomerOrderData order, InvoiceCalculator invoiceCalculator) {
        this.order = order;
    ...
    public void print() {
        ...
        System.out.println("\n" +
            String.format("%" + ((WIDTH + "INVOICE".length()) / 2) + "s", "INVOICE") + "\n" +
            bar + "\n" +
            String.format("%" + WIDTH + "s", datetime) + "\n" +
            String.format("%" + WIDTH + "s", "No: " + getInvoiceNumber()) + "\n" +
            bar + "\n" +
            order.getItemName() + " | " + order.getAmount() + "\n" +
            bar + "\n" +
            formatValue("Sub Total", invoiceCalculator.getSubTotal()) + "\n" +
            formatValue("Sales Tax (" + InvoiceCalculator.SALES_TAX + "%)", invoiceCalculator.getSalesTax()) + "\n" +
            formatValue("Card Fees", invoiceCalculator.getCardFees()) + "\n" +
